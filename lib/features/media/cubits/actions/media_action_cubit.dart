...
class MediaActionCubit extends Cubit<MediaActionState> {
  ...
  void resetSelectedImages() {
    selectedImages.clear();
    multiSelectionImages.clear();
    resetCheckBox();
    emit(MediaActionInitial());
    //debugPrint('Selected Images Length: ${selectedImages.length}');
  }

  void resetSingleImageCheckBox(String image) {
    try {
      final imageModel = multiSelectionImages.firstWhere(
        (element) => element.url.toLowerCase() == image.toLowerCase(),
      );

      imageModel.isSelected = false;
      multiSelectionImages.remove(imageModel);
      selectedImages.remove(imageModel);

      emit(ToggleImageCheckBox(imageModel));
    } catch (e) {
      debugPrint('Image not found in selectedImages: $image');
    }
  }
  ...
  void initSelectedImages(List<ImageModel> images, MediaCategory folder) {
